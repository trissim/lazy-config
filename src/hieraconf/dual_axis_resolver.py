...
def resolve_field_inheritance_old(obj, field_name: str, available_configs: dict[str, Any]) -> Any:
    """
            ...
            try:
                ...
                if value is not None:
                    if field_name == "well_filter":
                        logger.debug(
                            f"🔍 CONTEXT: Found concrete value in merged context {obj_type.__name__}.{field_name}: {value}"
                        )
                    return value
    ...
    try:
        ...
        if value is not None:
            if field_name == "well_filter":
                logger.debug(
                    f"🔍 INSTANCE: Found concrete value on instance {obj_type.__name__}.{field_name}: {value}"
                )
            return value
    except AttributeError:
        ...
        if type(config_instance) == obj_type:
            try:
                ...
                if field_value is not None:
                    ...
                    if field_name == "well_filter":
                        logger.debug(
                            f"🔍 FIELD-SPECIFIC BLOCKING: {obj_type.__name__}.{field_name} = {field_value} (concrete) - blocking inheritance"
                        )
                    return field_value
            except AttributeError:
                continue
    ...
    if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
        logger.debug(
            f"🔍 RESOLVING {obj_type.__name__}.{field_name} - checking context and inheritance"
        )
                ...
                if type(config_instance) == parent_type:
                    try:
                        ...
                        if value is None:
                            ...
                            logger.debug(
                                f"Inherited from blocking class {parent_type.__name__}: {value}"
                            )
                            return value
                    except AttributeError:
                        # Field doesn't exist on this config type
        ...
        for config_name, config_instance in available_configs.items():
            if type(config_instance) == parent_type:
                try:
                    ...
                    if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                        logger.debug(
                            f"🔍 Y-AXIS INHERITANCE: {parent_type.__name__}.{field_name} = {value}"
                        )
                    if value is not None:
                        if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                            logger.debug(
                                f"🔍 Y-AXIS INHERITANCE: FOUND {parent_type.__name__}.{field_name}: {value} (returning)"
                            )
    ...
    for config_name, config_instance in available_configs.items():
        ...
        if _is_related_config_type(obj_type, config_type):
            ...
            if config_type == obj_type:
                if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                    logger.debug(
                        f"🔍 CROSS-DATACLASS: Skipping self-inheritance from {config_type.__name__}"
                    )
                continue
            ...
            try:
                ...
                if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                    logger.debug(
                        f"🔍 CROSS-DATACLASS: {config_type.__name__}.{field_name} = {value} (related config)"
                    )
                if value is not None:
                    if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                        logger.debug(
                            f"🔍 CROSS-DATACLASS: FOUND {config_type.__name__}.{field_name}: {value}"
                        )
                    logger.debug(
                        f"Cross-dataclass inheritance from {config_type.__name__}: {value}"
                    )
                    return value
            except AttributeError:
                # Field doesn't exist on this config type
                if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                    logger.debug(
                        f"🔍 CROSS-DATACLASS: {config_type.__name__} has no field {field_name}"
                    )
                continue
        else:
            if field_name in ["output_dir_suffix", "sub_dir"]:
                logger.debug(
                    f"🔍 CROSS-DATACLASS: {config_type.__name__} not related to {obj_type.__name__}"
                )
    ...
    if blocking_class:
        try:
            ...
            if class_default is not None:
                logger.debug(
                    f"Using class default from blocking class {blocking_class.__name__}: {class_default}"
                )
...
def _is_related_config_type(obj_type: type, config_type: type) -> bool:
    ...
    if not issubclass(config_type, obj_type):  # config_type is NOT a child of obj_type
        ...
        obj_ancestors = set(
            cls for cls in obj_type.__mro__[1:] if is_dataclass(cls)
        )  # Skip obj_type itself
        config_ancestors = set(
            cls for cls in config_type.__mro__[1:] if is_dataclass(cls)
        )  # Skip config_type itself
...
def resolve_field_inheritance(obj, field_name: str, available_configs: dict[str, Any]) -> Any:
    """
        ...
        if type(config_instance) == obj_type:
            try:
                ...
                if field_value is not None:
                    if field_name == "well_filter":
                        logger.debug(
                            f"🔍 CONCRETE VALUE: {obj_type.__name__}.{field_name} = {field_value}"
                        )
                    return field_value
            except AttributeError:
                continue
        ...
        for config_name, config_instance in available_configs.items():
            if type(config_instance) == mro_class:
                try:
                    ...
                    if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                        logger.debug(
                            f"🔍 MRO-INHERITANCE: {mro_class.__name__}.{field_name} = {value}"
                        )
                    if value is not None:
                        if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                            logger.debug(
                                f"🔍 MRO-INHERITANCE: FOUND {mro_class.__name__}.{field_name}: {value} (returning)"
                            )
                        return value
                except AttributeError:
                    continue
    ...
    try:
        ...
        if class_default is not None:
            if field_name in ["output_dir_suffix", "sub_dir", "well_filter"]:
                logger.debug(
                    f"🔍 CLASS-DEFAULT: {obj_type.__name__}.{field_name} = {class_default}"
                )
            return class_default
    except AttributeError:
        pass
        ...
        for cls in config_class.__mro__:
            if hasattr(cls, field_name):
                ...
                if class_attr_value is not None:
                    has_override = True
                    logger.debug(
                        f"Class override check {config_class.__name__}.{field_name}: found concrete value {class_attr_value} in {cls.__name__}, has_override={has_override}"
                    )
                    return has_override

        # No concrete value found in any class in the MRO
        logger.debug(
            f"Class override check {config_class.__name__}.{field_name}: no concrete value in MRO, has_override=False"
        )
        return False
    except AttributeError:
...
def _find_blocking_class_in_mro(base_type: type, field_name: str) -> type | None:
    """
    ...
    for i, cls in enumerate(base_type.__mro__):
        ...
        if _has_concrete_field_override(cls, field_name):
            ...
            if has_parent_with_field:
                logger.debug(
                    f"Found blocking class {cls.__name__} for {base_type.__name__}.{field_name} (blocks parent inheritance)"
                )
                return cls
            else:
                logger.debug(
                    f"Class {cls.__name__} has concrete override but no parents with field - not blocking"
                )
    return None
